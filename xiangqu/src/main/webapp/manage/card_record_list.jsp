	...
	<div class="main-wrap">
		...
		<div class="crumb-wrap">
			<div class="crumb-list">
				...
				<span class="crumb-step">&gt;</span><span class="crumb-name">卡密记录管理</span>
			</div>
		</div>
		<form action="./card_record_list" method="post" name="pageForm" id="pageForm">
				...
				<div class="result-content">
					<table class="result-tab" width="100%">
						...
						<c:forEach items="${list}" var="p">
							<tr>
								<td class="tc">${p.card_num}</td>
								<td class="tc">${p.name}</td>
								<td class="tc">${p.phone }</td>
								<td class="tc">${p.address }</td>
								<td class="tc"><fmt:formatDate value="${p.addtime }" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
							</tr>
						</c:forEach>
