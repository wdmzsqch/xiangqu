...
// lolaArray[0] = "120.064269,30.871983";
// lolaArray[1] = "120.064269,30.859822";
// lolaArray[2] = "120.064269,30.847661";
// lolaArray[3] = "120.072326,30.871983";
// lolaArray[4] = "120.072326,30.859822";
// lolaArray[5] = "120.072326,30.847661";
// lolaArray[6] = "120.080384,30.871983";
// lolaArray[7] = "120.080384,30.859822";
// lolaArray[8] = "120.080384,30.847661";
var font = 2;
var bold = 0;
var fontSize = 12;
var zoom = 15;
...
function setadpoint(array, index){
	...
	if(index == 20){
		...
		$.ajax({
			type: "post",
			url: "makepointmap2",
			data: {keywords :keywords, areaSearch :areaSearch, SourceClass :SourceClass, longitude :longitude, latitude: latitude},
...
function makepointmap2(){
	...
	//20 16
// 	for(var i=0; i<3; i++){
// 		var longitude = parseFloat(120.067348+0.045*i).toFixed(6); 
// 		for(var j=0; j<3; j++){
// 		var latitude = parseFloat(30.892429-0.028*j).toFixed(6); 
// 			$.ajax({
// 				type: "post",
// 				url: "makepointmap2",
// 				data: {keywords :keywords, areaSearch :areaSearch, SourceClass :SourceClass, longitude :longitude, latitude: latitude},
...
function makepointmap(){
	...
	$.ajax({
		type: "post",
		url: "makepointmap",
		data: {keywords :keywords, areaSearch :areaSearch, SourceClass :SourceClass},
		success: function(msg){
			//第1个点中心点经纬度
			...
			//第21个点中心点经纬度
			var longitude21 = 120.13446808; 
			var latitude21 = 30.85250035;
			var data = eval("("+msg+")");
			...
			var href21 = "http://restapi.amap.com/v3/staticmap?location="+longitude21+","+latitude21;
			href21 += "&zoom="+zoom+"&size=1024*1024";
			if(list21 != null && list21.length > 0){
				for(var w = 0; w< list21.length; w++){
					//标签内容，字符最大数目为15
					var content = list21[w].plotName;
					if(content > 15){
						content = content.substring(0,16);
					}
					var longitude = list21[w].longitude;
					var latitude = list21[w].latitude;
					var background = list21[w].background;
					if(background == ""){
						background = "0x008000";
					}
					if(w == 0){
						href21 += "&labels="+content+","+font+","+bold+","+fontSize+","+fontColor+","+background+":"+longitude+","+latitude;
					}else{
						href21 += "|"+content+","+font+","+bold+","+fontSize+","+fontColor+","+background+":"+longitude+","+latitude;
					}
				}
			}
			href21 += "&key="+key;
...
function setMarkSoloPoint(id,array, index){
	if(index == 20){
		...
		$.ajax({
			...
			data: {ids :id, longitude :longitude, latitude :latitude},
			success: function(msg){
				var data = eval("("+msg+")");
				var list = data.list;
				var returnlongitude = data.returnlongitude;
...
<body>
	...
	<div class="main-wrap">
		...
		<form action="./ad_point_list" method="post" name="pageForm" id="pageForm">
		...
		<div class="result-wrap">
			<div class="result-title">
					<div class="result-list">
						<input class="btn btn-primary btn2" name="creat_task" value="新建广告点位" type="button" onclick="javascript:location='./ad_point_detail'">
				...
				<table class="result-tab" width="100%">
					...
					<c:forEach items="${list}" var="ad">
						<tr>
							...
							<td><a class="link-del" href="./ad_point_detail?id=${ad.id}">查看详情</a>
								<br/>
