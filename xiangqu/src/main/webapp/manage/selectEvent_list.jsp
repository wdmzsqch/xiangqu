...
<body>
	...
	<div class="main-wrap">
		...
		<form action="./selectEvent" method="post" name="pageForm" id="pageForm">
					...
					<table class="result-tab" width="100%">
						...
						<c:forEach items="${event_list}" var="event">
							<tr>
								...
								<td>${event.name}</td>
								<td><fmt:formatDate value="${event.startTime}" type="date" pattern="yyyy-MM-dd HH:mm:ss" />至<fmt:formatDate value="${event.endTime}" type="date" pattern="yyyy-MM-dd HH:mm:ss"/></td>
								<td><fmt:formatDate value="${event.createTime}" type="date" pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td>
								<img src="${fileUrl}${event.pic }" width="60px" height="60px">
								</td>
								<td><a class="link-del" href="./chooseActivity?relatived_id=${event.id}">选择</a>
								</td>
							</tr>
						</c:forEach>
